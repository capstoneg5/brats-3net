...
# -----------------------------
def compute_diffs(rows, query_lesion_id):
    ...
    # find query lesion
    q = next(
        (r for r in rows if r.get("lesion_id") == query_lesion_id),
        rows[0] if rows else {},
    )

    q_et = safe_float(q.get("et_pct"), 0.0)
    q_tc = safe_float(q.get("tc_pct"), 0.0)

    out = []

    for r in rows:
        et = safe_float(r.get("et_pct"), 0.0)
        tc = safe_float(r.get("tc_pct"), 0.0)
