...
def step_train_embed(cfg: RunConfig) -> None:
    ...
    cubes_dir = cfg.artifacts_root / "lesions" / "cubes"
    if not cubes_dir.exists():
        raise RuntimeError(f"Lesion cubes not found: {cubes_dir}. Run --extract_lesions first.")

    from src.embeddings.train_embedder_3d import TrainCfg, train
    try:
        import torch
    ...
    train_cfg = TrainCfg(
        cubes_dir=cubes_dir,
        out_dir=cfg.embed_checkpoint.parent,
        epochs=20,
        batch_size=16,
        lr=1e-3,
        embed_dim=768,
    ...
    best_path = train(train_cfg)
    if Path(best_path) != cfg.embed_checkpoint:
        shutil.copy2(best_path, cfg.embed_checkpoint)
    _log(f"  - embed_checkpoint={cfg.embed_checkpoint}")
    _log("✅ train_embed complete")
...
def step_embed_all(cfg) -> None:
    ...
    if not cfg.embed_checkpoint.exists():
        raise RuntimeError(
            f"Embedder checkpoint not found: {cfg.embed_checkpoint}. "
            "Run --train_embed first or pass --embed_checkpoint."
        )

    EMBED_DIM = 768
    from src.embeddings.embedder_3d import TrainedCubeEmbedder
    embedder = TrainedCubeEmbedder(cfg.embed_checkpoint)
    ...
    with cfg.embeddings_jsonl.open("w", encoding="utf-8") as f:
        for path in cube_files:
            ...
            record = {
                ...
                "dim": EMBED_DIM,
                "metadata": {
                    "source": "trained_embedder_3d",
                    "cube_file": path.name,
                    "cube_shape": [int(x) for x in cube.shape],
                },
            }
            f.write(json.dumps(record, default=lambda o: o.tolist() if hasattr(o, 'tolist') else str(o)) + "\n")
            written += 1
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    run.add_argument("--neo4j_password", default=os.getenv("NEO4J_PASSWORD"))
