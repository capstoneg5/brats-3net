...
def _guess_role(p: Path) -> str:
    ...
    if {"labelstr", "labelsts", "labels", "masks", "segmentations"} & parent_parts:
        return "seg"
...
def _looks_like_label_map(x: np.ndarray) -> bool:
    """Heuristic for segmentation masks stored without 'seg' in the filename."""
    x = np.nan_to_num(x)
    uniq = np.unique(x)
    if uniq.size == 0 or uniq.size > 10:
        return False
    if not np.allclose(uniq, np.round(uniq), atol=1e-5):
        return False
    return float(np.max(uniq)) <= 10.0
...
def route_for_chat(
    ...
    if img_path is None:
        seg_only_path = _find_seg(mods)
        if seg_only_path is not None and len(nifti_files) == 1:
            img_path = seg_only_path
        else:
            return RouteResult(
                kind="nifti",
    ...
    if mask_source == "gt_seg":
        ...
        if seg_path is not None:
            ...
            # Common BraTS mini/test layouts store segmentation under labelsTr/labelsTs
            # with names like BRATS_003.nii.gz, which do not include "seg".
            if len(nifti_files) == 1 and _looks_like_label_map(image):
                mask = image.copy()
                mask_info["inferred_from_single_nifti"] = True
                mask_info["inference_reason"] = "single_nifti_looks_like_label_map"
            else:
